	...
	e([ 'jquery' ], function(e) {
		return (function() {
			...
			function c(t) {
				var i = m();
				return (
					v || n(i), t && 0 === e(':focus', t).length ? void h(t) : void (v.children().length && v.remove())
				);
			}
			function d(t) {
			...
			function g(t) {
				...
				function l() {
					t.message &&
						(M.append(x.escapeHtml ? i(t.message) : t.message).addClass(x.messageClass), y.append(M));
				}
				function u() {
				...
				function b() {
					clearTimeout(H),
						(q.hideEta = 0),
						y.stop(!0, !0)[x.showMethod]({ duration: x.showDuration, easing: x.showEasing });
				}
				function D() {
				...
				if (
					('undefined' != typeof t.optionsOverride &&
						((x = e.extend(x, t.optionsOverride)), (E = t.optionsOverride.iconClass || E)),
					!g(x, t))
				) {
					T++, (v = n(x, !0));
					var H = null,
...
(function(factory) {
	...
	var _defaultValues = { animations: { name: 'rotate_right', time: '2000ms' }, fade: [ 400, 200 ] };
	...
	function Show(container, settings) {
		...
		if (data === false) {
			...
			overlay = $('<div>', { class: 'loadingoverlay' })
				.css(_css.overlay)
			...
			if (data.wholePage) {
				overlay.css({ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%' });
			}
			...
			if (settings.image) {
				if ($.isArray(settings.imageColor)) {
					if (settings.imageColor.length === 0) {
						...
						settings.imageColor = { fill: settings.imageColor[0] };
					} else {
						settings.imageColor = { fill: settings.imageColor[0], stroke: settings.imageColor[1] };
					}
				} else if (settings.imageColor) {
					settings.imageColor = { fill: settings.imageColor };
				}
				...
				if (
					...
					$.ajax({ url: settings.image, type: 'GET', dataType: 'html', global: false }).done(function(data) {
						element.html(data);
						element.children().css(_css.element_svg);
						if (!settings.imageClass && settings.imageColor) element.find('*').css(settings.imageColor);
					});
			...
			if (settings.fontawesome) {
				...
				$('<div>', { class: settings.fontawesome }).appendTo(element);
						...
						.css(
							...
							data.progress.margin
								? data.progress.margin.value +
									(data.progress.margin.fixed ? data.progress.margin.units : '%')
								: 0
						);
	...
	function Hide(container, force) {
		...
		if (force || data.count <= 0) {
			overlay.animate({ opacity: 0 }, data.settings.fade[1], function() {
	...
	function Progress(container, value) {
		...
		if (value === false) {
			...
			if (v < 0) v = 0;
			if (v > 100) v = 100;
			data.progress.bar.show().animate({ width: v + '%' }, data.progress.speed);
		}
	}
	...
	function _CreateElement(overlay, order, autoResize, resizeFactor, animation) {
		var element = $('<div>', { class: 'loadingoverlay_element', css: { order: order } })
			.css(_css.element)
			.data({ loadingoverlay_autoresize: autoResize, loadingoverlay_resizefactor: resizeFactor })
			.appendTo(overlay);
	...
	function _ParseSize(value) {
		if (!value || value < 0) {
			...
			return { fixed: true, units: value.slice(-4), value: value.slice(0, -4) };
		} else if (typeof value === 'string' && [ 'rem' ].indexOf(value.slice(-3)) > -1) {
			return { fixed: true, units: value.slice(-3), value: value.slice(0, -3) };
		} else if (
			typeof value === 'string' &&
			[ 'px', 'em', 'cm', 'mm', 'in', 'pt', 'pc', 'vh', 'vw' ].indexOf(value.slice(-2)) > -1
		) {
			return { fixed: true, units: value.slice(-2), value: value.slice(0, -2) };
		} else {
			return { fixed: false, units: 'px', value: parseFloat(value) };
		}
	}
	$(function() {
		$('head').append(
		...
		_setupCodecs: function() {
			...
			return (
				(e._codecs = {
					...
					m4a: !!(o.canPlayType('audio/x-m4a;') || o.canPlayType('audio/m4a;') || o.canPlayType('audio/aac;'))
						.replace(/^no$/, ''),
					mp4: !!(o.canPlayType('audio/x-mp4;') || o.canPlayType('audio/mp4;') || o.canPlayType('audio/aac;'))
						.replace(/^no$/, ''),
				...
				var h = function() {
					...
					v.gain.setValueAtTime(e, n.ctx.currentTime),
						...
						void 0 === v.bufferSource.start
							? i._loop ? v.bufferSource.noteGrainOn(0, _, 86400) : v.bufferSource.noteGrainOn(0, _, s)
							: i._loop ? v.bufferSource.start(0, _, 86400) : v.bufferSource.start(0, _, s),
					...
					: ((a._orientation = [ e, n, t, r, o, i ]),
						void 0 !== a.ctx.listener.forwardX
							? (a.ctx.listener.forwardX.setTargetAtTime(e, Howler.ctx.currentTime, 0.1),
								...
								a.ctx.listener.upX.setTargetAtTime(e, Howler.ctx.currentTime, 0.1),
								a.ctx.listener.upY.setTargetAtTime(n, Howler.ctx.currentTime, 0.1),
								a.ctx.listener.upZ.setTargetAtTime(t, Howler.ctx.currentTime, 0.1))
						...
						(o._pannerAttr = {
							...
							coneOuterAngle:
								void 0 !== n.pannerAttr.coneOuterAngle
									...
									: o._coneOuterAngle,
							coneOuterGain:
								void 0 !== n.pannerAttr.coneOuterGain ? n.pannerAttr.coneOuterGain : o._coneOuterGain,
							distanceModel:
								void 0 !== n.pannerAttr.distanceModel ? n.pannerAttr.distanceModel : o._distanceModel,
							maxDistance:
							...
							rolloffFactor:
								void 0 !== n.pannerAttr.rolloffFactor ? n.pannerAttr.rolloffFactor : o._rolloffFactor,
							panningModel:
...
/* eload.js */
function subvpayment() {
	var vpayment = $('#vpayment').val();
	var vip = $('#vip').val();
	var vmac = $('#vmac').val();
	$('#subvpayment').attr('disabled', true);
	$.ajax({
		type: 'POST',
		url: '/admin/index?pop=1&lpb=subvpayment',
		data: { vpayment: vpayment, vmac: vmac, vip: vip },
		success: function(msg) {
			if (msg == '1') {
				document.getElementById('vpayment').value = '';
				toastr.success('Voucher code ' + vpayment + ' successfully redeemed', '', {
					timeOut: '1000',
					positionClass: 'toast-top-right'
				});
				$('#subvpayment').attr('disabled', false);
			} else {
				document.getElementById('vpayment').value = '';
...
function eloadproduct() {
	var mac = $('#mac').val();
	var ip = $('#ip').val();
	$.LoadingOverlay('show');
	var eloadnumber = $('#eloadnumber').val();
	if (eloadnumber != '') {
		$.post('admin/index?pop=1&lpb=eloadproduct', { eloadnumber: eloadnumber, ip: ip, mac: mac }, function(data) {
			$('#eloadannouncer').html(data);
			$('#eloadproduct').modal({ backdrop: 'static', keyboard: false });
			$.LoadingOverlay('hide');
		});
	} else {
		location.reload();
...
var sound = new Howl({ src: [ 'assets/images/email-notif.mp3' ] });
var eloadinsertcoin = new Howl({ src: [ 'assets/images/insertcoin.mp3' ] });
var eloadnarrate = new Howl({ src: [ 'assets/images/narrate.mp3' ] });
function balanceinquiry() {
	var mac = $('#mac').val();
	var ip = $('#ip').val();
	var ieloadnumber = $('#ieloadnumber').val();
	$.LoadingOverlay('show');
	if (ieloadnumber != '') {
		$.post('admin/index?pop=1&lpb=balanceinquiry', { ieloadnumber: ieloadnumber, ip: ip, mac: mac }, function(
			data
		) {
			$('#balanceinquiry_announcer').html(data);
			$('#balancecheck').modal({ backdrop: 'static', keyboard: false });
			$.LoadingOverlay('hide');
		});
	} else {
		location.reload();
...
function proceload(ip, mac, mnumber, pcode, network, coinslot) {
	$.LoadingOverlay('show');
	$.post(
		'admin/index?pop=1&lpb=autoproc',
		{ ip: ip, mac: mac, mnumber: mnumber, pcode: pcode, network: network, coinslot: coinslot },
		function(data) {
			var success = data.trim();
			if (success == '1') {
				$('#eloadprocess').modal('hide');
				$('#eloadcomplete').modal('show');
				eloadnarrate.play();
				$.LoadingOverlay('hide');
			} else {
				$('#eloaderror').html(data);
				$.LoadingOverlay('hide');
			}
		}
	);
}
function selectcoinslot(ip, mac, pcode, mnumber, network, category, price, pdescription) {
	var radiuscode = $('#radiuserverid').val();
	$.LoadingOverlay('show');
	if (radiuscode != '') {
		$.post(
			'admin/index?pop=1&lpb=selectcoinslot',
			{
				ip: ip,
				mac: mac,
				pcode: pcode,
			...
			function(data) {
				$('#multicoinslot_announce').html(data);
				$('#eloadmulticoinslot').modal({ backdrop: 'static', keyboard: false });
			}
		);
	}
}
function eloadprocess(ip, mac, pcode, mnumber, network, category, coinslot) {
	var radiuscode = $('#radiuserverid').val();
	$.LoadingOverlay('show');
	if (radiuscode != '') {
		$.post(
			'admin/index?pop=1&lpb=eloadprocess',
			{
				ip: ip,
				mac: mac,
				pcode: pcode,
			...
			function(data) {
				$('#eloadprocess_announce').html(data);
				$('#eloadprocess').modal({ backdrop: 'static', keyboard: false });
				var tamount = '';
				if (typeof EventSource !== 'undefined') {
					var IrequestSource = new EventSource('admin/index?insertcoin=1&ip=' + ip + '&mac=' + mac + '');
					IrequestSource.addEventListener(
						'tamount',
						function(event) {
							if (tamount != event.data) {
								tamount = event.data;
								document.getElementById('eloadamount').innerHTML = tamount;
								sound.play();
								isend(coinslot);
							}
						},
				...
				if (!$('#eloadproduct').hasClass('show')) {
					eloadinsertcoin.play();
					$.LoadingOverlay('hide');
				}
			}
		);
...
function isend(coinslot) {
	$.ajax({ type: 'GET', url: 'admin/index?isend=1&func=isend&coinslot=' + coinslot + '' });
}
function getnumbers(myfield, e, dec) {
	var key;
	var keychar;
	...
	if (key == null || key == 0 || key == 8 || key == 9 || key == 27 || keychar == '.') return true;
	if (key == 13) {
		if (popvisible == 0) {
			get_products();
			popvisible = 1;
...
function applyvoucher(vcode, time) {
	if (confirm('Apply voucher: ' + vcode + ' (' + time + ')?')) {
		$('#vcode').val(vcode);
		$('#sub-voucher').trigger('click');
	} else {
		return false;
	}
}
$('#pfunc').click(function(e) {
	$.ajax({
		type: 'POST',
		url: '/admin/index?pfunc=1',
		success: function(msg) {
			if (msg == '1') {
				toastr.success('You Sucessfully Pause your time!', '', {
					timeOut: '1000',
					positionClass: 'toast-top-right'
				});
				location.reload();
			} else if (msg == '2') {
				toastr.success('You Sucessfully Resume your time!', '', {
					timeOut: '1000',
					positionClass: 'toast-top-right'
				});
				location.reload();
...
var timer = parseInt($('div[usertime]').attr('usertime'));
$('#btn-vcode').click(function(e) {
	var amountminutes = '';
	var minutes = '';
	amountminutes = $('#amountminutes').val();
	minutes = $('#minutes').val();
	$.ajax({
		type: 'POST',
		url: '/admin/index?sconvert=1',
		data: { amountminutes: amountminutes },
		success: function(msg) {
			if (msg == '1') {
				toastr.success('Sucessfully Convert Time To Voucher!', '', {
					timeOut: '1000',
					positionClass: 'toast-top-right'
				});
				var r = document.getElementById('minutes');
				r.value = minutes - amountminutes;
				timer = timer - amountminutes * 60;
			} else {
				toastr.error(msg, 'Error');
			}
		}
...
$('div[usertime]').each(function() {
	var countdown = setInterval(function() {
		...
		if (timer >= 86400) {
			var result =
				'<b> ' + days + '</b> d. <b>' + hours + '</b> hr.<b>' + minutes + '</b>min. <b>' + seconds + '</b> sec';
			...
			var result = '<h2 class="paused ptime"> -- </h2>';
		}
		$('#utimer').html(result);
	}, 1000);
});
...
$('#close-eload').on('click', function(e) {
	$('.eload-spinner').attr('disabled', false);
});
$('#buyload').on('hide.bs.modal', function(e) {
	$('.eload-spinner').attr('disabled', false);
	$('.fa-spin').remove();
});
$('#eloadcomplete').on('hide.bs.modal', function(e) {
	location.reload();
});
$('.eload-spinner').on('click', function(e) {
	e.preventDefault();
	$('.eload-spinner').attr('disabled', 'disabled');
	$(this).prepend('<i class="fa fa-spinner fa-spin"></i> ');
});
$('#sub-voucher').click(function(e) {
	var vcode = '';
	vcode = $('#vcode').val();
	$('#sub-voucher').attr('disabled', true);
	$.ajax({
		type: 'POST',
		...
		success: function(msg) {
			if (msg == '1') {
				...
				toastr.success('Voucher code ' + vcode + ' successfully redeemed', '', {
					timeOut: '1000',
					positionClass: 'toast-top-right'
				});
	...
	$.ajax({
		...
		success: function(msg) {
			if (msg == '1') {
				...
				toastr.success('Time Logout!', '', { timeOut: '1000', positionClass: 'toast-top-right' });
				location.reload();
			}
		}
	});
...
function iclaim(coinslot) {
	$.ajax({ type: 'GET', url: '/admin/index?isend=1&func=iclaim&coinslot=' + coinslot + '' });
}
function timeconvertion($minutes) {
